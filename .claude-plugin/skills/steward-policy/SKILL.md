...
1. Identify the target project and whether the host is Codex or Claude Code from the active session. Ask only if either is unresolved. Locate `bin/steward.mjs` at the toolkit root, two directories above this skill folder. Verify that file exists, inspect its `--help`, and check Node 22+ and any target-project runtime pins. Do not assume a global command.
2. For a marketplace installation, use the actual plugin root reported by the host and pass `--plugin` to both `init` and `install`. This uses the bundled skills without copying duplicates. For a repository clone, omit `--plugin`. Keep the toolkit at a stable path; after a plugin update or relocation, use its new root to rerun setup.
