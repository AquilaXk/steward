...
folders use `skills/steward-<name>/SKILL.md`. Installation copies those sources into
native discovery locations and adds only the host-specific invocation metadata needed.
...
| recall | Material prior context → relevant sourced facts and open questions | No repeated exhaustive memory sweep; CLI listing is not indexed search |
